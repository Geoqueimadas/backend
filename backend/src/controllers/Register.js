...
async function index(req, res) {
  try {
    const query = await pool
      .query(`SELECT * FROM "usercadastro"`)
...
async function store(req, res) {
  ...
  try {
    ...
    const query = await pool.query(
      `
                INSERT INTO "usercadastro" ("name", "email", "datanasc", "telefone", "senha", "cep", "rua", "numero", "bairro", "cidade", "uf") 
                VALUES('${nome}', '${email}', '${dataNasc}', '${telefone}', '${senha}', '${cep}', '${rua}', '${num}', '${bairro}', '${cidade}', '${uf}')
            `
    );
...
async function delet(req, res) {
  ...
  try {
    const query = await pool.query(
      `DELETE FROM "usercadastro" WHERE "id" = ${parseInt(id)}`
    );
  ...
  try {
    const query = await pool.query(
      `UPDATE "Cadastro" SET ("Nome", "Email", "Data_nasc", "Telefone",
            "Senha", "Cep", "Rua", "Num", "Bairro", "Cidade", "UF") = 
            ('${nome}', '${email}', '${dataNasc}', '${telefone}', '${senha}', '${cep}', '${rua}', '${num}', '${bairro}', '${cidade}', '${uf}') 
            WHERE "ID" = ${id}`
    );
